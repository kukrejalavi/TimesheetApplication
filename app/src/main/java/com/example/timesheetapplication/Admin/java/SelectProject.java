...
public class SelectProject extends AppCompatActivity {
        ...
        savebtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!txtselecteditems.getText().toString().isEmpty()) {
                    ...
                    returnInt.putExtra("resultnames", nameslist.toString().replace("[", "").replace("]", ""));
                    ...
                    Validations.Toast(getApplicationContext(),"Please Select Project");
            }
        });
    ...
    public void DataLoad(){
        ...
        if(intent!= null) {
            ...
            if(intent.getStringExtra("costcenter") != null) {

            Firebaselinks.costcenterfirebaselink().orderByChild("costcenter").equalTo(intent.getStringExtra("costcenter")).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if(dataSnapshot.getChildrenCount() > 0){
                        for(DataSnapshot child : dataSnapshot.getChildren()){
                            emplist.add(child.child("project").getValue().toString());
                        }
                        dataAdapter = new ArrayList<>();
                        Firebaselinks.projectsfirebaselink().addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                if (dataSnapshot.getChildrenCount() > 0) {
                                    for (DataSnapshot child : dataSnapshot.getChildren()) {
                                        if (!emplist.get(0).contains(child.child("projname").getValue().toString())) {
                                            if (child.child("projstatus").getValue().toString().equals("Active")) {
                                                isi = new DataAdapter(child.child("projname").getValue().toString(), child.child("projid").getValue().toString(), "", false);
                                                dataAdapter.add(isi);
                                            }
                                        }
                                    }
